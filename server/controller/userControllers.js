...
export const sendVerifyMail = async (username, email, user_id) => {
  try {
    ...
    const mailOption = {
      ...
      html: `<p>Hii ${username}, please click <a href="http://localhost:5173/VerifyMail/${user_id}">here</a> to verify your email.</p>`,
    };
   ...
   console.log(phone);
    ...
    if(userPhone){
     const token= generateToken(userPhone)
      res.json({status:true,message:'User Found',token:token,name:userPhone.name})
    }else{
