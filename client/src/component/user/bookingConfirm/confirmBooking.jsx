...
function confirmBooking() {
  ...
  const [showitem, setshowitem] = useState(0) //stripe
    ...
    const options = {
      ...
      handler: function (response) {
        ...
        if (response.razorpay_payment_id && response.razorpay_signature) {
          userAxios.post('/verifyRazorpay', { response: response, requestData })
            .then((res) => {
              if (res.data.status) {
  ...
  const  submitForm = async (e) => {
    ...
    if (!isChecked) {
      setErrMsg({ id: 11, message: "Please agree to the terms and conditions." });
      return;
    }
    else{
      // setshowitem(1)
      const response = await  userAxios.post('/razorpay',
      {selectedPayment},
      {
        headers: {
      ...
      <div className="mb-24">
        ...
        <form method="POST" onSubmit={submitForm}>
            ...
            >
              <div className="2nd main p-5 sm:p-10 md:p-5">
                ...
                <div className="flex justify-between lg:m-4 xl:m-5 pb-8 sm:pb-14">
                  <h2 className="text-sm lg:text-lg font-medium">Profession</h2>
                    <h2 className="lg:text-lg font-mono">
                      {proData.category.name}
                    </h2>
                </div>
                <div className="bg-green-500 p-3 m-auto sm:p-5 md:p-3 lg:p-5 rounded-xl">
                  <div className="flex justify-between mb-3">
                    ...
                    <h4 className="font-medium">₹{proData.charge.fulltime}</h4>
                  </div>
            ...
            <div>
              <p style={{ color: "red" }}>{ErrMsg.message}</p>
                ...
                <div className="flex justify-between p-5 sm:pt-10">
                  ...
                  <h2 className="text-lg font-bold font-mono">₹1500</h2>
                </div>
          ...
          className={`w-4 h-4 border me-2 mt-1  rounded  ${
            ErrMsg.id==11 ? 'focus:ring-red-600 border-red-800 bg-red-50 focus:ring-3' : 'focus:ring-blue-600 bg-gray-50 focus:ring-3 border-gray-300 dark:ring-offset-gray-800'
          }`}
